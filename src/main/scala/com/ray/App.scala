...
object App {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val v_rfsm_nationality = MySQLUtil.readTable("v_rfsm_nationality")
  ...
  def writeDatabase(): Unit = {
    val df_all = spark.read.format("csv").option("header", "true")
      .load("C:\\Users\\a6481\\Documents\\Courseware\\GraduationDesign\\data\\data6.csv")
    //更改列名
    val df = df_all.withColumnRenamed(df_all.columns(0), "pid")
  ...
  def createAnalysisView(): Unit = {
    ...
    val v_analysis_ttfa = demographic.join(v_ttfa_nationality, Seq("pid"))
      ...
      .join(t_dim_loe, Seq("LOECode"))
      .join(t_dim_occupation, Seq("occupationCode"))
      .select("pid", "genderName", "ageName", "LOEName", "occupationName", "TTFAName")
      .orderBy("pid")
    ...
    val v_analysis_toad = demographic.join(v_toad_nationality, Seq("pid"))
      ...
      .join(t_dim_loe, Seq("LOECode"))
      .join(t_dim_occupation, Seq("occupationCode"))
      .select("pid", "genderName", "ageName", "LOEName", "occupationName", "TOADName")
      .orderBy("pid")
    ...
    val v_analysis_cfda = demographic.join(v_cfda_nationality, Seq("pid"))
      ...
      .join(t_dim_loe, Seq("LOECode"))
      .join(t_dim_occupation, Seq("occupationCode"))
      .select("pid", "genderName", "ageName", "LOEName", "occupationName", "CFDAName")
      .orderBy("pid")
    ...
    val v_analysis_rfda = demographic.join(v_rfda_nationality, Seq("pid"))
      ...
      .join(t_dim_loe, Seq("LOECode"))
      .join(t_dim_occupation, Seq("occupationCode"))
      .select("pid", "genderName", "ageName", "LOEName", "occupationName", "RFDAName")
      .orderBy("pid")
    ...
    val v_analysis_rfsu = demographic.join(v_rfsu_nationality, Seq("pid"))
      ...
      .join(t_dim_loe, Seq("LOECode"))
      .join(t_dim_occupation, Seq("occupationCode"))
      .select("pid", "genderName", "ageName", "LOEName", "occupationName", "RFSUName")
      .orderBy("pid")
